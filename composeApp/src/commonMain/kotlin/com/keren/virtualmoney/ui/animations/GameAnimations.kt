...
@Composable
fun Modifier.shake(enabled: Boolean = false, shakeIntensity: Float = 10f): Modifier {
    if (!enabled) return this
...
@Composable
fun Modifier.spin(enabled: Boolean = true, durationMs: Int = 3000): Modifier {
    if (!enabled) return this
    ...
    val rotation by
            infiniteTransition.animateFloat(
                    ...
                    targetValue = 360f,
                    animationSpec =
                            infiniteRepeatable(animation = tween(durationMs, easing = LinearEasing))
            )
...
fun BounceIn(
    ...
    AnimatedVisibility(
            ...
            exit =
                    scaleOut(targetScale = 0f, animationSpec = tween(AnimationDurations.FAST)) +
                            fadeOut(),
            modifier = modifier
    ) { content() }
}
