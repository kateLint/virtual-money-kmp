...
import com.keren.virtualmoney.game.EnhancedGameEngine
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        when (val state = gameState) {
            ...
            is GameState.Finished ->
                    FinishedScreen(state = state, onTryAgain = { gameEngine.resetGame() })
        }
    }
}
...
private fun ReadyScreen(
    ...
    Column(
        ...
        Button(onClick = onStartGame, modifier = Modifier.size(width = 200.dp, height = 60.dp)) {
...
private fun RunningScreen(
    ...
    Box(modifier = Modifier.fillMaxSize().onSizeChanged { screenSize = it }) {
        ...
        // HUD Overlay
        GameHUD(timeRemaining = state.timeRemaining, score = state.score)

        // Coins overlay
        CoinOverlay(coins = state.coins, skin = CoinSkin.CLASSIC, screenSize = screenSize, onCoinTapped = onCoinTapped)
    }
}

/** Finished state - shows final score and try again button. */
@Composable
private fun FinishedScreen(state: GameState.Finished, onTryAgain: () -> Unit) {
    Column(
            modifier =
                    Modifier.fillMaxSize()
        ...
        Text(text = "Final Score", fontSize = 24.sp, color = Color.White)
        ...
        Button(onClick = onTryAgain, modifier = Modifier.size(width = 200.dp, height = 60.dp)) {
...
@Composable
private fun GameHUD(timeRemaining: Int, score: Int) {
