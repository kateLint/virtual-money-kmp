...
class GameRepository(private val authManager: AuthManager) {
    ...
    suspend fun joinMatchmaking(gameMode: GameMode, displayName: String): Result<Unit> {
        return try {
            ...
            val queueEntry =
                    mapOf("odId" to userId, "displayName" to displayName, "joinedAt" to now)
    ...
    /** Update team score in Team Battle mode. */
    suspend fun updateTeamScore(gameId: String, teamId: Int, score: Int): Result<Unit> {
        return try {
            gamesRef().child(gameId).child("teamScores").child(teamId.toString()).setValue(score)
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    /** Get real-time player count in matchmaking queue. */
    suspend fun getQueueCount(gameMode: GameMode): Int {
        return try {
            val snapshot = matchmakingRef().child(gameMode.name).valueEvents.first()
    ...
    /** Observe queue count for a game mode. */
    fun observeQueueCount(gameMode: GameMode): Flow<Int> {
        return matchmakingRef().child(gameMode.name).valueEvents.map { snapshot ->
            snapshot.children.count()
        }
    }
